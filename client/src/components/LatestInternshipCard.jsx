...
const LatestInternshipCard = ({job}) => {
    const navigate = useNavigate();
    const  bgarr = [intern1,intern2,intern3,intern4];
    ...
    return (
        <div onClick={()=> navigate(`/description/${job._id}`)} className=' rounded-md shadow-xl bg-white border border-gray-100 cursor-pointer hover:scale-[1.02]  transition-transform duration-300 ease-in-out'>
           ...
           <div className="bg-cover bg-center h-24 w-full  rounded-t-md  relative " style={{ backgroundImage: `url(${intern1})` }}>
            ...
            <div className='flex items-center gap-2 mt-4 p-2 pb-5'>
                ...
                <Badge className={'text-[#F83002] font-bold text-[2px] '} variant="ghost">{job?.jobType}</Badge>
