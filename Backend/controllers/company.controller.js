...
export const deleteCompany = async (req, res) => {
  try {
    ...
    const logoUrl = company.logo; // Ensure your DB stores the full Cloudinary URL
 
    if (logoUrl) {
      const publicId = logoUrl.split('/').pop().split('.')[0]; // Extract public ID
    ...
    // 🔴 Now, delete company from database
    await Company.findByIdAndDelete(companyId);

    return res.status(200).json({ message: "Company and Releted Jobs are deleted", success: true });
